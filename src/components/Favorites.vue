<template>
    ...
    <ul>
        <li v-for="item in favorites" :key="item.id" class="mb-3"><a :href="'https://www.youtube.com/watch?v=' + item.videoId" target="_blank"><img :src="item.thumbnail" width="90" />【{{ item.videoId }}】{{ item.title }}</a>
            <el-button type="success" plain @click="eliminateFavo(item.id)" class="ml-2">解除</el-button>
        </li>
    </ul>
</template>
